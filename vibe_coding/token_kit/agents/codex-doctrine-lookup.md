...
1. Establish the lane directory. The caller gives it; if it does not, use
   `$TK_DATA_ROOT/agrescap/tasks/<campaign>/lanes/codex-doctrine-lookup/v0`
...
3. Make EXACTLY ONE dispatch call:
   ...
   ~/.config/token_kit/bin/codex-dispatch \
     ...
     --cwd <lane-dir> \
     --task-file <lane-dir>/codex/doctrine-lookup/prompt.txt \
     --out <lane-dir>/codex/doctrine-lookup/answer.md
...
4. Read the exit code before anything else.
   ...
   - rc 42 -> codex is UNAVAILABLE here: the binary is absent,
     ...
     Report exactly `CODEX_LANE row=doctrine-lookup status=REFUSE
